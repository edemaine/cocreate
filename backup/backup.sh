...
## Additional flags, e.g. to omit history which is much larger than boards
MONGO_DUMP_FLAGS="--excludeCollection=objects.diff"

## Local backup directory name
BACKUP_DIR=cocreate-backup
...
#CLOUD_REMOTE=cocreate-backup

## Directory to create on cloud remote.
CLOUD_DIR="cocreate-backup$datedir"

cd "`dirname "$0"`"
echo \* mongodump
ssh "$SSH_REMOTE" mongodump --db "$MONGO_DB" --gzip $MONGO_DUMP_FLAGS
echo \* rsync
