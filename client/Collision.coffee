...
intersectsSpecific = 
  pen: (aabb, obj) ->
    ...
    for i in [0...(obj.pts.length - 1)]
      ...
      # Fatten line into rectangle consisting of points pt0 -> pt1 -> -pt0 -> -pt1 in counterclockwise order
      ...
      for [p0, p1, fx, fy] in [
        ...
        p2 = {x: testPt.x + fx * aabb.width() / 2, y: testPt.y + fy * aabb.height() / 2}
  ...
  rect: (aabb, obj) ->
    ...
    fattened = obj.aabb.fattenedXY ((aabb.width() - obj.width) / 2), ((aabb.height() - obj.width) / 2)
    testPt = aabb.center()

    if fattened.containsPoint testPt
      # Inside. Take fill or lack of fill into account.
      obj.fill != null or !(obj.aabb.fattened -obj.width).contains aabb
    else
      # Outside. Take roundedness into account.
      testPt.x -= obj.aabb.center().x
      testPt.y -= obj.aabb.center().y
      testPt.x = Math.max ((Math.abs testPt.x) - fattened.width() / 2), 0
      testPt.y = Math.max ((Math.abs testPt.y) - fattened.height() / 2), 0
  ...
  ellipse: (aabb, obj) ->
    ...
    # Test outer ellipse
    collapsed =
      x: Math.max ((Math.abs testPt.x) - aabb.width() / 2), 0
      y: Math.max ((Math.abs testPt.y) - aabb.height() / 2), 0
    if collapsed.x * collapsed.x / ((obj.aabb.width() / 2) * (obj.aabb.width() / 2)) + \
      collapsed.y * collapsed.y / ((obj.aabb.height() / 2) * (obj.aabb.height() / 2)) > 1
      return false

    if obj.fill == null
      # Test inner ellipse using Minkowski difference,
      # which in this case is the intersection of 4 ellipses
